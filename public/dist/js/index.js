...
**************************************************/
	...
	// Mobile Edit Btn
	this.$editBtnAll = this.$body.querySelector('.user-nav__edit-btn');
	this.$cancelBtnAll = this.$body.querySelector('.user-nav__btn--cancel');
	this.$saveBtnAll = this.$body.querySelector('.user-nav__btn--save');
	// Swipeable
	...
	swipeContent: function (val) {
		...
		if(self.$swipeable.className != 'animate'){
			this.$swipeableEl.addEventListener('touchstart',function(e) {
				...
				if (self.$editBtnAll.contains(e.target)) {
					console.log('Edit Mobile');
					self.$swipeable.classList.add('edit');
					self.$userEditItem.forEach( e => {
						e.classList.remove('user-edit__tooltip--active');
						let label = e.querySelector('.tooltip__title');
						let inputValue = e.querySelector('.user-edit__value');
						let input = e.querySelector('.tooltip__input');
						input.value = inputValue.textContent;
						let hasText = (element) => {
							( element.value.length > 0) ? label.classList.add('tooltip__title--filled') : label.classList.remove('tooltip__title--filled');
						}
						hasText(input);
						input.addEventListener('focus', function() {
							label.classList.add('tooltip__title--focused');
						});
						input.addEventListener('focusout', function() {
							label.classList.remove('tooltip__title--focused');
						});
				...
				if (self.$saveBtnAll.contains(e.target)) {
					self.$userEditItem.forEach( e => {
						e.querySelector('.user-edit__value').textContent = e.querySelector('.tooltip__input').value;
					});
					self.$swipeable.classList.remove('edit');
					return;
				};
				if (self.$cancelBtnAll.contains(e.target)) {
					self.$swipeable.classList.remove('edit');
					return;
				};
				(self.$swipeable.className.indexOf('animate') > -1 || self.$swipeable.className.indexOf('edit') > -1) ? self.$preventTouch = false : self.$preventTouch = true;

	...
	**************************************************/
	...
	this.controller = 'productVariant';
...
ProductVariant.prototype = {
	...
	**************************************************/
	initController: function (mode) {
		...
		// 	self.generate($('.productVariant' + mode), i, item);
		// });
		self.populate();
		main.overlay.hide();
	},
	...
	**************************************************/
	getRelated: function (companyID, callback) {
		...
		$.ajax({
			...
			console.log(data.collection);
			self.collection = data.collection;
			var key = 'productVariantID-' + companyID;
			if (Array.isArray(self.collection) && self.collection.length != 0) {
				window.localStorage.setItem([key], JSON.stringify(self.collection));
				main.functionsObj['productVariant'] = productVariant;
			}
	...
	itemForm: function (container, callback) {
		...
		.append($('<form />', { 'class': 'new-variant' })
			...
			.append($('<div />', { 'class': 'uploadedAvatar hidden' })
				.append($('<label />', { 'class': 'form-control-label', 'for': 'avatar', text: 'Imagen' }))
				.append($('<div />', { 'class': 'input-group' })
			...
			.append($('<div />', { 'class': 'col-xs-12 col-sm-12 col-md-12 col-lg-12' })
				.append($('<label />', { 'class': 'form-control-label', 'for': 'barcode', text: 'barcode' }))
				.append($('<div />', { 'class': 'input-group barcode', 'id': 'barcode' })
					.append($('<input />', { 'class': 'form-control pull-right required', 'type': 'text', 'data-name': 'barcode', 'name': 'titlename' }))
				)
			)
			.append($('<div />', { 'class': 'col-xs-6 col-sm-6 col-md-6 col-lg-6' })
				.append($('<label />', { 'class': 'form-control-label', 'for': 'item_content', text: 'Contenido' }))
				.append($('<div />', { 'class': 'input-group category', 'id': 'item_content' })
					.append($('<input />', { 'class': 'form-control pull-right required', 'type': 'text', 'data-name': 'item_content', 'name': 'item_content' }))
				)
			)
			.append($('<div />', { 'class': 'col-xs-6 col-sm-6 col-md-6 col-lg-6' })
				.append($('<label />', { 'class': 'form-control-label', 'for': 'sort', text: 'Palabras Claves' }))
				.append($('<div />', { 'class': 'input-group sort', 'id': 'sort' })
					.append($('<span />', { 'class': 'input-group-addon' })
						.prepend($('<i />', { 'class': 'fa fa-balance-scale' }))
					)
					.append($('<input />', { 'class': 'form-control pull-right required', 'type': 'text', 'data-name': 'sort', 'name': 'sort' }))
				)
			)
			.append($('<div />', { 'class': 'col-xs-12 col-sm-12 col-md-12 col-lg-12' })
		...
		$(document).on('click', '.new-brand .changeAvatar', function() {
			$('#avatar').trigger('click');
		});
		$(document).on('change', '.new-brand #avatar', function() {
			main.selectedImage(this, $('.new-brand .uploadAvatar img'));
		});
		...
		$('.product-brand-btn').on('click', function(){
			...
			addnew.product = $('form.new-brand').serializeArray();
			addnew.productobj = [];
			addnew.productobj.push({
				id: 'new-brand',
				titlename: $('form.new-brand .titlename input').val(),
				category: $('form.new-brand .category input').val(),
				measure: $('form.new-brand .measure input').val(),
				description: $('form.new-brand .description input').val(),
				avatar: [{
					file: $('form.new-brand .uploadedAvatar .file').val()
				}]
			});
			$('form.new-brand').remove();
			$(document).off('click', '.new-brand .changeAvatar');
			$(document).off('change', '.new-brand #avatar');
